  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <Card className="w-full max-w-md p-6 space-y-4 relative">
        <Button variant="ghost" size="icon" className="absolute top-2 right-2 text-zinc-400 hover:text-white" onClick={onClose}>
          <X size={20} />
        </Button>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            {photo ? (
              <div className="relative group overflow-hidden rounded-xl border border-white/10 aspect-video bg-zinc-950 flex items-center justify-center">
                <img
                  src={photo}
              ...
              <label className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/10 bg-white/[0.02] hover:bg-white/[0.04] p-6 text-center cursor-pointer transition group">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/5 text-zinc-400 group-hover:text-emerald-300 group-hover:bg-emerald-300/10 transition">
                  <Camera size={20} />
                </div>
                <div className="mt-2.5">
                  <p className="text-xs font-semibold text-white">Upload a progress photo</p>
                  <p className="mt-0.5 text-[10px] text-zinc-500">Supports PNG, JPG, or WEBP</p>
                </div>
                <input type="file" accept="image/*" onChange={handlePhotoChange} className="hidden" />
              </label>
            )}
